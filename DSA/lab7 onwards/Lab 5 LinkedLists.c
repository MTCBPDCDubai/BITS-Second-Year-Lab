...
struct Student * insertAtIndex(struct Student * current,struct Student * node,int index){
...
int size(struct Student * current){
    int count=0;
    while(current!=NULL){
        count++;
        current=current->next;
...
void search(struct Student * current,char key[]){
    while(current!=NULL){
...
void main(){
    ...
    while(ch!=8){
        ...
        if(ch==1){
            ...
            int index;
            printf("\nSpecify the index: ");scanf("%d",&index);
